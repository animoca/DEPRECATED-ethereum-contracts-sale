...
describe('Sale', function () {
  ...
  async function doUpdateSkuPricing(params = {}) {
    ...
    if (params.useErc20) {
      ...
      this.erc20Token = await ERC20.new([params.owner || owner], [params.erc20TotalSupply || erc20TotalSupply], registry.address, forwarder.address, {
        from: params.owner || owner,
      });
  ...
  describe('start()', function () {
    ...
    it('reverts if called by any other than the contract owner', async function () {
      await expectRevert(doStart.bind(this)({owner: purchaser}), 'Ownable: not the owner');
    });
  ...
  describe('pause()', function () {
    ...
    it('reverts if called by any other than the contract owner', async function () {
      await doStart.bind(this)();
      await expectRevert(this.contract.pause({from: purchaser}), 'Ownable: not the owner');
    });
  ...
  describe('unpause()', function () {
    ...
    it('reverts if called by any other than the contract owner', async function () {
      await doStart.bind(this)();
      await this.contract.pause({from: owner});
      await expectRevert(this.contract.unpause({from: purchaser}), 'Ownable: not the owner');
    });
  ...
  describe('updateSkuPricing()', function () {
    ...
    it('reverts if called by any other than the contract owner', async function () {
      await expectRevert(doUpdateSkuPricing.bind(this)({owner: purchaser}), 'Ownable: not the owner');
    });

    it('reverts if `tokens` and `prices` have different lengths', async function () {
      const ethTokenAddress = await this.contract.TOKEN_ETH();

      await expectRevert(this.contract.updateSkuPricing(sku, [ethTokenAddress], [], {from: owner}), 'Sale: inconsistent arrays');
    });
    ...
    it("should update a SKU's set of supported token prices", async function () {
      ...
      const registry = await artifacts.require('ForwarderRegistry').new();
      const forwarder = await artifacts.require('UniversalForwarder').new();
      const erc20Token = await ERC20.new([owner], [erc20TotalSupply], registry.address, forwarder.address, {from: owner});
  ...
  describe('_setTokenPrices()', function () {
    ...
    it('reverts if the sku token price set exceeds the defined tokens per-sku capacity', async function () {
      const registry = await artifacts.require('ForwarderRegistry').new();
      const forwarder = await artifacts.require('UniversalForwarder').new();
      const erc20Token1 = await ERC20.new([owner], [erc20TotalSupply], registry.address, forwarder.address, {from: owner});
      const erc20Token2 = await ERC20.new([owner], [erc20TotalSupply], registry.address, forwarder.address, {from: owner});
      const erc20Token3 = await ERC20.new([owner], [erc20TotalSupply], registry.address, forwarder.address, {from: owner});
      await expectRevert(
