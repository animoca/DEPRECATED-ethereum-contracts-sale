  ...
  if (!_fixture) {
    _fixture = async ([owner], provider) => {
      ...
      for (const [key, data] of Object.entries(tokens)) {
        if (key == 'WETH') {
          data.contract = await data.abstraction.new({from: owner});
        } else {
          // ERC20 Mock
          data.contract = await data.abstraction.new([owner], [data.supply], registry.address, forwarder.address, {from: owner});
        }
      }
